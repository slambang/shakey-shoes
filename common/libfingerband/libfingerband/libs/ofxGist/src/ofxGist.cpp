...
//------------------------------------------------------------------
void ofxGist::processAudio(const float * input, int bufferSize, int nChannels, int sampleRate){
    ...
    gist.processAudioFrame(input, bufferSize);
...
void ofxGist::processOnsetDetection(GIST_FEATURE feature){
    ...
    if(bigChange == 1 && !_isNoteOn){
        event.energy = gist.peakEnergy();
        event.frequency = gist.pitch();
        event.feature = feature;
        event.note = getNoteFrequency();
        event.onsetAmount = _values[feature];
...
string  ofxGist::getNoteName(){
    ...
    return n[relVal]+"-"+"FIX_NOTE_TO_STRING_HERE";/*ofToString(oct);*/
