...
class BluetoothDeviceEntityMapper {
    ...
    fun map(entities: List<BluetoothDeviceEntity>) = entities.map { map(it) }
    ...
    private fun requireProduct(id: Int) =
        PRODUCT_MAPS[id] ?: throw IllegalArgumentException("Required product mapping $id")

    companion object {
        private val PRODUCT_MAPS = mapOf(
